...
def main():
    ...
    while frames < 100000:
        ...
        while(1):
            ...
            if (math.sqrt((goalX[0] - robot_x)**2 + (goalX[1] - robot_y)**2)) <= 30:
                if frames_termimal == 0:
                    frames_termimal += 1
                    position_array_aug = np.array([[robot_x/600, robot_y/600, robot_phi/np.pi]])
                    controls_array_aug = np.array([[v/0.5, omega/0.02]])
                    obstacles_array_aug = np.array([obs])
                else:
                    frames_termimal += 1
                    position_array_aug = np.append(position_array_aug, np.array([[robot_x/600, robot_y/600, robot_phi/np.pi]]),axis = 0)
                    controls_array_aug = np.append(controls_array_aug, np.array([[v/0.5, omega/0.02]]),axis = 0)
                    obstacles_array_aug = np.append(obstacles_array_aug, np.array([obs]),axis = 0)
            ...
            position_array = np.append(position_array, np.array([[robot_x/600,robot_y/600,robot_phi/np.pi]]),axis = 0)
    ...
    print(position_array_aug.shape)
    print(controls_array_aug.shape)
    print(obstacles_array_aug.shape)

    folder_path = "data_aug"

    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    
    np.savetxt("data_aug/positions.csv", position_array, delimiter=",",fmt ='% s')
    np.savetxt("data_aug/controls.csv", controls_array, delimiter=",",fmt ='% s')
    np.savetxt("data_aug/obstacles.csv", obstacles_array, delimiter=",",fmt ='% s')

    np.savetxt("data_aug/positions_aug.csv", position_array_aug, delimiter=",",fmt ='% s')
    np.savetxt("data_aug/controls_aug.csv", controls_array_aug, delimiter=",",fmt ='% s')
    np.savetxt("data_aug/obstacles_aug.csv", obstacles_array_aug, delimiter=",",fmt ='% s')
