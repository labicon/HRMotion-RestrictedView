...
trajectory = np.loadtxt("data3/positions.csv", delimiter=",", dtype=float)
controls = np.loadtxt("data3/controls.csv", delimiter=",", dtype=float)
obstacles = np.loadtxt("data3/obstacles.csv", delimiter=",", dtype=float)

combined = np.concatenate((controls, obstacles), axis=1)

class Denoiser(nn.Module):
    def __init__(self, width):
        super().__init__()
        self.net = nn.Sequential(nn.Linear(7+1, width), nn.ReLU(),
...
# torch.(denoiser.state_dict(), 'checkpoints/diffusion3_checkpoint.pth')

# torch.load(denoiser.state_dict(), 'checkpoints/diffusion3_checkpoint.pth')

denoiser.load_state_dict(torch.load("checkpoints/diffusion3_checkpoint.pth"))
...
def main():
    ...
    while(1):
        ...
        if(len(close_obst) == 0) or (math.sqrt((goalX[0] - robot_x)**2 + (goalX[1] - robot_y)**2) <= min(dist)):           # No obstacle in sensor skirt
            ...
            obs = torch.tensor([[0.0, 0.0, 0.0, 0.0, robot_phi/np.pi]]).float()
        # Paranoid behavior - run away from obstacle
            ...
            obs = torch.tensor([[1.0, (obs_x - robot_x)/600, (obs_y - robot_y)/600, distance/skirt_r, robot_phi/np.pi]]).float()
