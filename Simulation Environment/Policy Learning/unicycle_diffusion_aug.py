...
trajectory = np.loadtxt("data_aug/positions.csv", delimiter=",", dtype=float)
controls = np.loadtxt("data_aug/controls.csv", delimiter=",", dtype=float)
obstacles = np.loadtxt("data_aug/obstacles.csv", delimiter=",", dtype=float) 

combined = np.concatenate((controls,trajectory[0:-1,:], obstacles), axis=1)

trajectory_aug = np.loadtxt("data_aug/positions_aug.csv", delimiter=",", dtype=float)
controls_aug = np.loadtxt("data_aug/controls_aug.csv", delimiter=",", dtype=float)
obstacles_aug = np.loadtxt("data_aug/obstacles_aug.csv", delimiter=",", dtype=float)

combined_aug = np.concatenate((controls_aug,trajectory_aug, obstacles_aug), axis=1)
...
for epoch in range(nb_epochs):
    ...
    if random.random() < 0:
        x0 = torch.tensor(np.array(random.choices(combined_aug, k=batch_size))).float().view(-1, combined_aug.shape[1])
    else:
...
torch.save(denoiser.state_dict(), 'checkpoints/diffusion_checkpoint_aug.pth')
