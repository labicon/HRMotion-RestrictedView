...
def main():
    ...
    while frames < 100000:
        ...
        while(1):
            ...
            if(len(close_obst) == 0) or (math.sqrt((goalX[0] - robot_x)**2 + (goalX[1] - robot_y)**2) <= min(dist)):           # No obstacle in sensor skirt
                ...
                obs = np.array([0.0, 0.0, 0.0, 0.0, robot_phi/np.pi])
            # Paranoid behavior - run away from obstacle
                ...
                obs = np.array([1.0, (obs_x - robot_x)/600, (obs_y - robot_y)/600, distance/skirt_r, robot_phi/np.pi])
    ...
    np.savetxt("data3/positions.csv", position_array, delimiter=",",fmt ='% s')
    np.savetxt("data3/controls.csv", controls_array, delimiter=",",fmt ='% s')
    np.savetxt("data3/obstacles.csv", obstacles_array, delimiter=",",fmt ='% s')
