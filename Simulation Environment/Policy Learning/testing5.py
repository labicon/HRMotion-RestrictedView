...
data_directories = [
    ...
    './data/monroe_data/1/480/2024-02-27T170748/',
    './data/monroe_data/1/96/2024-02-27T170409/',
    './data/monroe_data/1/96/2024-02-27T170504/',
    './data/monroe_data/1/96/2024-02-27T170552/',
    './data/monroe_data/1/96/2024-02-27T170618/',
    './data/monroe_data/1/96/2024-02-27T170639/',
    './data/monroe_data/1/96/2024-02-27T170705/',
...
# Calculate the expected grid dimensions
...
for train_index, val_index in kf.split(train_occupancy_maps):
    ...
    val_loader = DataLoader(TensorDataset(torch.tensor(train_occupancy_maps[val_index], dtype=torch.float32).to(device), torch.tensor(train_actions[val_index], dtype=torch.float32).to(device)), batch_size=batch_size, shuffle=False)
...
actual_trajectory = [(test_trajectory_data[0]['x'], test_trajectory_data[0]['y'])]
...
predicted_trajectories = generate_trajectories(model, diffusion, initial_state, goal, test_obstacle_data, noise_scale=0.8)
...
def update(idx):
    ...
    actual_xs, actual_ys = zip(*actual_trajectory)
    ax2.plot(actual_xs, actual_ys, label='Actual Trajectory', marker='o', color='blue')

    # Plot predicted trajectories in global view
    for i, predicted_trajectory in enumerate(predicted_trajectories):
        predicted_xs, predicted_ys = zip(*predicted_trajectory)
        ax2.plot(predicted_xs, predicted_ys, label=f'Predicted Trajectory {i+1}', marker='x')
        
        # Plot the detailed predicted trajectory up to the current index
        if idx < len(predicted_xs):
            ax1.plot(predicted_xs[:idx+1], predicted_ys[:idx+1], label=f'Predicted Trajectory {i+1}', marker='x')
    ...
    # Process obstacles for the current step
    obstacles_in_view = []
    ...
    if obstacles_in_view:
        ...
        ax1.scatter(xs, ys, s=dot_size, c='red', label='Viewable')
    if non_viewable_obstacles:
        xs, ys = zip(*non_viewable_obstacles)
        ax1.scatter(xs, ys, s=dot_size, c='gray', label='Non-viewable')
    ax1.legend()
    ...
    ax2.plot([agent_x, wedge_global_start[0]], [agent_y, wedge_global_start[1]], 'b--')
    ax2.plot([agent_x, wedge_global_end[0]], [agent_y, wedge_global_end[1]], 'b--')
    ax2.add_patch(Wedge((agent_x, agent_y), view_range, agent_theta * 180/np.pi - view_angle/2, agent_theta * 180/np.pi + view_angle/2, alpha=0.2, color='blue'))

    if global_obstacles_in_view:
        xs, ys = zip(*global_obstacles_in_view)
        ax2.scatter(xs, ys, s=dot_size, c='red', label='Viewable')
    if global_obstacles_non_viewable:
        xs, ys = zip(*global_obstacles_non_viewable)
        ax2.scatter(xs, ys, s=dot_size, c='gray', label='Non-viewable')
    ax2.scatter(agent_x, agent_y, s=dot_size, c='blue', marker='o', label='Agent')  # Agent's position
    ax2.legend()
    
    # Plot the moving actual trajectory up to the current index
    if idx < len(actual_xs):
        ax2.plot(actual_xs[:idx+1], actual_ys[:idx+1], label='Actual Trajectory', marker='o', color='blue')

ani = animation.FuncAnimation(fig, update, frames=len(test_trajectory_data), repeat=False)

# Save the animation as a GIF to the local machine
...
# Function to visualize the occupancy map for a single data point
def visualize_occupancy_map(occupancy_map, agent_x, agent_y, agent_theta, obstacles, view_range, view_angle, grid_size, grid_height, grid_width):
    fig, ax = plt.subplots(1, 2, figsize=(12, 6))
    
    # Plot occupancy map
    ax[0].imshow(occupancy_map.reshape((grid_height, grid_width)), cmap='gray', origin='lower')
    ax[0].set_title('Occupancy Map')
    ax[0].set_xlabel('Grid X')
    ax[0].set_ylabel('Grid Y')
    
    # Plot global view with agent and obstacles
    ax[1].set_xlim(agent_x - view_range, agent_x + view_range)
    ax[1].set_ylim(agent_y - view_range, agent_y + view_range)
    ax[1].set_title('Global View')
    ax[1].set_xlabel('Global X')
    ax[1].set_ylabel('Global Y')
    
    # Draw agent
    ax[1].plot(agent_x, agent_y, 'bo', label='Agent')
    ax[1].arrow(agent_x, agent_y, view_range * np.cos(agent_theta), view_range * np.sin(agent_theta), color='blue', head_width=10, head_length=10)
    
    # Draw view cone
    wedge_start = transform_to_global_frame(view_range * np.cos(np.deg2rad(-view_angle/2)), view_range * np.sin(np.deg2rad(-view_angle/2)), agent_x, agent_y, agent_theta)
    wedge_end = transform_to_global_frame(view_range * np.cos(np.deg2rad(view_angle/2)), view_range * np.sin(np.deg2rad(view_angle/2)), agent_x, agent_y, agent_theta)
    ax[1].plot([agent_x, wedge_start[0]], [agent_y, wedge_start[1]], 'b--')
    ax[1].plot([agent_x, wedge_end[0]], [agent_y, wedge_end[1]], 'b--')
    
    # Plot obstacles
    for obstacle in obstacles:
        local_x, local_y = transform_to_local_frame(obstacle['x'], obstacle['y'], agent_x, agent_y, agent_theta)
        global_x, global_y = obstacle['x'], obstacle['y']
        ax[1].plot(global_x, global_y, 'rx', label='Obstacle')

        if is_within_view(local_x, local_y, view_range, view_angle):
            ax[1].plot(global_x, global_y, 'go', label='Viewable Obstacle')
        else:
            ax[1].plot(global_x, global_y, 'ro', label='Non-viewable Obstacle')

    plt.legend()
    plt.show()

# Visualize the occupancy map for a single data point
index_to_visualize = 10  # Change this to visualize different data points

agent_x = train_trajectory_data[index_to_visualize]['x']
agent_y = train_trajectory_data[index_to_visualize]['y']
agent_theta = train_trajectory_data[index_to_visualize]['theta']
obstacles = train_obstacle_data[index_to_visualize]
occupancy_map = train_occupancy_maps[index_to_visualize]

visualize_occupancy_map(occupancy_map, agent_x, agent_y, agent_theta, obstacles, view_range, view_angle, grid_size, grid_height, grid_width)
