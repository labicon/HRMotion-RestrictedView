...
def black_box_function(x,y,n):
    ...
    obs_theta = y
    for i in range(200):
        current_tra = np.loadtxt("data/orig_traj/positions_param_"+str(n)+"_"+str(i)+".csv", delimiter=",")
        obstacles = np.loadtxt("data/orig_traj/obstacles_"+str(n)+"_"+str(i)+".csv", delimiter=",")
...
def black_box_function_pred(x,y,l):
    #traj_i_array = np.array([170,  27, 159, 135,   3,  17,  59, 140,  98, 114])
    #traj_i_array = np.array([5])
    sigma = x
    epsilone = 1e-1
    ...
    obs_theta = y
    for i in range(200):
        current_tra = np.loadtxt("data/pred_traj/positions_param_"+str(l)+"_"+str(i)+".csv", delimiter=",")
...
def main():
    ...
    params = np.array([[55.0, 0.815],[99.0, 1.157],[22.0, 1.047],[70.0, 0.524]])

    prob_list_orig = np.zeros(4)
    prob_list_pred = np.zeros(4)

    for l in range(4):
        prob_list_orig[l] = black_box_function(params[l,0],params[l,1],l)
        prob_list_pred[l] = black_box_function_pred(params[l,0],params[l,1],l)

    print(prob_list_orig)
    print(prob_list_pred)
