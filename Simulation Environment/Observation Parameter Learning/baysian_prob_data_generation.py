...
def main():
    ...
    prob_list = np.array([0.2,0.4,0.6,0.8])

    for prob in [prob_list[-1]]:

        print(prob)

        if not os.path.exists("prob_data/prob_"+str(prob)):
            os.makedirs("prob_data/prob_"+str(prob))
        ...
        for n in range(500):
            ...
            while(1):
                ...
                if distance_to_goal < threshold or frames > 5000:
                    ...
                    np.savetxt("prob_data/prob_"+str(prob)+"/positions_"+str(n)+".csv", position_array, delimiter=",",fmt ='% s')
