...
def angle_truth_value(robot_x, robot_y, robot_theta, obs_theta, circ_x, circ_y, r, distance_to_obstacle):
    relative_angle = math.atan2(circ_y - robot_y, circ_x - robot_x)
    truth_value = False
    if abs(relative_angle - robot_theta) <= obs_theta:
        truth_value = True
    elif abs(relative_angle - robot_theta ) <= obs_theta + math.atan2(r, distance_to_obstacle):
        truth_value = True
    return truth_value
...
def main():
    ...
    for n in range(200):
        ...
        # Robot
        robot_x = 100; robot_y = 100; robot_phi = 0; robot_theta = 0; robot_l = 15; robot_b = 6  # Initial position
        position_array = np.array([[robot_x, robot_y, robot_phi, robot_theta]])
        ...
        obstacles = np.loadtxt("data_2/obstacles_0_09_"+str(n)+".csv", delimiter=",")
        ...
        while(1):
            ...
            bot = robot(robot_x, robot_y, robot_phi, robot_theta, robot_l, robot_b, data)
            ...
            #                                             (int(bot.x + skirt_r*math.cos(bot.theta + obs_theta)), int(bot.y + skirt_r*math.sin(bot.theta + obs_theta))) ,
            #                                             (int(bot.x + skirt_r*math.cos(bot.theta - obs_theta)), int(bot.y + skirt_r*math.sin(bot.theta - obs_theta)))])
            ...
            for i in range(num_circ_obsts):
                ...
                if( distance <= (skirt_r + radius[i]) and angle_truth_value(robot_x, robot_y, robot_theta, obs_theta, circ_x[i], circ_y[i],radius[i],distance)):
            ...
            #if frames%20 == 0:
            robot_theta = np.random.normal(robot_phi,0.9); robot_theta = (robot_theta + np.pi) % (2 * np.pi) - np.pi
            position_array = np.append(position_array, np.array([[robot_x,robot_y,robot_phi,robot_theta]]),axis = 0)
            ...
            if distance_to_goal < threshold or frames > 5000:
                ...
                np.savetxt("data_2/positions_pred_0_09_"+str(n)+".csv", position_array, delimiter=",",fmt ='% s')
                #np.savetxt("data_2/obstacles_0_09_"+str(n)+".csv", obstacles, delimiter=",",fmt ='% s')
                break
