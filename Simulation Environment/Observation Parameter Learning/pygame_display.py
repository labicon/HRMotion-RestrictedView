...
def main():
    ...
    for n in range(1):
        ...
        while(1):
            ...
            pygame.draw.polygon(screen, (255, 219, 187), [(int(bot.x), int(bot.y)),
                                                        (int(bot.x + skirt_r*math.cos(bot.phi + obs_theta)), int(bot.y + skirt_r*math.sin(bot.phi + obs_theta))) ,
                                                        (int(bot.x + skirt_r*math.cos(bot.phi - obs_theta)), int(bot.y + skirt_r*math.sin(bot.phi - obs_theta)))])
            draw_circular_obsts(num_circ_obsts, radius, circ_x, circ_y, (128,128,128))
            ...
            pygame.display.flip()
            # save the pygame display as image
            pygame.image.save(screen, "pygame_display/display.png")
            break
            # frames+=1
