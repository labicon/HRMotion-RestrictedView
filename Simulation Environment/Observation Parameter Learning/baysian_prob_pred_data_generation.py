...
def draw_circular_obsts(num, radius, circ_x, circ_y, color):
    for i in range(num):
        pygame.draw.circle(None, color, (circ_x[i], circ_y[i]), radius[i], 0)
...
def main():
    ...
    if not os.path.exists("prob_data"):
        os.makedirs("prob_data")

    prob_list = np.array([0.2209, 0.4352, 0.6140, 0.8249])
    prob_list_main = np.array([0.2, 0.4, 0.6, 0.8])

    for l in range(4):

        print(prob_list)

        if not os.path.exists("prob_data/prob_"+str(prob_list[l])):
            os.makedirs("prob_data/prob_"+str(prob_list[l]))
        ...
        for n in range(500):
            ...
            data = {"screen":None, "goalX":goalX, "vmax":0.5, "gtg_scaling":0.0001, "K_p":0.01, "ao_scaling":0.00005}
            ...
            obstacles = np.loadtxt("prob_data/prob_"+str(prob_list_main[l])+"/obstacles_"+str(n)+".csv", delimiter=",")
            ...
            while(1):
                ...
                if distance_to_goal < threshold or frames > 5000:
                    ...
                    np.savetxt("prob_data/prob_"+str(prob_list[l])+"/positions_pred"+str(n)+".csv", position_array, delimiter=",",fmt ='% s')
                    # np.savetxt("prob_data/prob_"+str(prob)+"/obstacles_"+str(n)+".csv", obstacles, delimiter=",",fmt ='% s')
                    break
