...
def read_traj(prob1,prob2,n):
    traj_1 = np.loadtxt("prob_data/prob_"+str(prob1)+"/positions_pred"+str(n)+".csv", delimiter=",")
    traj_2 = np.loadtxt("prob_data/prob_"+str(prob2)+"/positions_"+str(n)+".csv", delimiter=",")
...
def main():
    ...
    probs_2 = np.array([0.2, 0.4, 0.6, 0.8])
    probs_1= np.array([0.2209, 0.4352, 0.6140, 0.8249])
    ...
    for l in range(4):
        ...
        for i in range(200):
            ...
            [traj_1, traj_2] = read_traj(probs_1[l],probs_2[l],i)
            distance_arrays[l,i] = fast_frechet.distance(traj_1,traj_2)
    ...
    np.savetxt("prob_data/frechet_distance.csv", distance_arrays, delimiter=",")
...
def plot():
    distance_arrays = np.loadtxt("prob_data/frechet_distance.csv", delimiter=",")
    ...
    for l in range(4):
        ...
        ax.set_title(f'Probability {l+1}',fontsize=21)
