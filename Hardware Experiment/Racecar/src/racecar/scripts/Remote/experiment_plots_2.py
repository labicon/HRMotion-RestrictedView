...
def plot_at_time(t):
    ...
    traj = np.loadtxt("bicycle_diff_data/bicycle_full_positions_expt.csv", delimiter=",")
    print(traj.shape)
    ...
    for i in [t]:
        ...
        plt.plot([x1,x2],[y1,y2],color='orange',linewidth=10, label='Robot',zorder=1)
    ...
    plt.fill(x,y,'green',zorder=2,label='Goal')
    # target_marker = mlines.Line2D([], [], color='green', marker='o', linestyle='None', markersize=10, label='Goal')
    ...
    # plt.axis('equal')
    plt.savefig("plots/images/"+str(t)+".png")
    #plt.show()

if(__name__ == '__main__'):
    # main()
    #creat gif using the function plot_at_time for all timesteps

    timesteps = 17
    images = []
    for t in range(timesteps):
        # plt.savefig("plots/images/"+str(t)+".pdf",bbox_inches='tight')
        plot_at_time(t)
        images.append(imageio.imread("plots/images/"+str(t)+".png"))
        
    imageio.mimsave("plots/expt_animation.mp4", images, fps=2)
