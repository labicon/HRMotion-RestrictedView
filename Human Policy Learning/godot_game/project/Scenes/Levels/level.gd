extends Node2D
@onready var light480 = get_node("player1/r480")
@onready var light192 = get_node("player1/r192")
@onready var light96 = get_node("player1/r96")
...
func update_display():
    ...
    if score <= 0 or goal_reached:
        ...
        $RestartButton.show()
        $player1.save_all_trajectories(light_value, game_mode, game_selected, time)
        get_tree().paused = true
